...
def drawing_to_image_with_color_stroke_multi_channel(drawing, H, W, channels):
    cts = int(channels / 3)
...
def drawing_to_image_with_color_v2(drawing, H, W):
    ...
    return image
def drawing_to_image_with_color_v3(drawing, H, W):
    point = []
    time = []
    for t, (x, y) in enumerate(drawing):
        point.append(np.array((x, y), np.float32).T)
...
class QDDRDataset(Dataset):
    ...
    def __init__(self, csv_id=None, num_preclass=('110k', 200), time_length=3,mode='train',transform=None, isGetList=False):
        super(QDDRDataset, self).__init__()
        if mode in ['train', 'valid']:
            if csv_id is None:
                self.csv = pd.read_csv('./input/valid_{}_{}.csv'.format(num_preclass[0], 80))
            else:
                self.csv = pd.read_csv('./input/shuffle_csv_{}_{}/train_k{}.csv.gz'.format(num_preclass[1], num_preclass[0], csv_id))
        else:
            self.csv = pd.read_csv('./input/test_simplified.csv')

        self.dict_label = self.load_labels()
        self.countryCode = self.load_country()
    ...
    def load_country(self):
        label = pd.read_csv('./input/country.csv')
        labelName = label['countrycode'].tolist()
    ...
    def __getitem__(self, index):
        ...
        if self.mode in ['train', 'valid']:
            ...
            countryCode = self.csv['countrycode'][index]
            countryCode = self.countryCode[countryCode]
            if self.transform is not None:
                input = self.transform(drawing, label, recognized, countryCode)
            return input
            ...
            try:
                ...
                countryCode = 0
            if self.transform is not None:
                out = self.transform(drawing, countryCode)
                if len(out) == 2:
                    return out[0], out[1], str(name), countryCode
                else:
                    return out, str(name), countryCode
